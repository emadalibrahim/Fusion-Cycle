...
class model:
    def __init__(self, N_iteration=10, thresh=0.99, Num_ensembles = 5):
    ...
    def calc_x(self,x0,df,checkpoint_path_list):
        df = self.predict(checkpoint_path_list,df,pd.DataFrame(x0).to_numpy()) # predict gamma at x0
        x = np.exp(df['dHfus_pred']/(self.R) * ((1/df['MP_pred']) - (1/df['Temperature [K]'])) - (df['gamma']) ) 
    ...
    def calculate_solubility(self,df):
        ...
        for i in range(self.N_iteration): # Iterate to adjust for x
            x = self.calc_x(x,df,checkpoint_path_list)
